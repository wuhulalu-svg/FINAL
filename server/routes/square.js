...
router.post('/posts', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: result.lastID, message: '发布成功' });
  } catch (error) {
...
router.post('/posts/:id/comments', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: result.lastID, message: '评论成功' });
  } catch (error) {
...
router.delete('/posts/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      db.run('DELETE FROM square_posts WHERE id = ?', [id], function(err) {
        if (err) reject(err);
        resolve(this);
      });
    });
