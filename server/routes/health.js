        ...
        (err, existing) => {
            ...
            if (existing) {
                ...
                db.run(
                    ...
                    (err) => {
                        if (err) {
                            console.error('Insert health record error:', err);
                            return res.status(500).json({ error: 'Internal server error' });
                        }
...
// 获取单条健康记录
router.get('/records/:date', authenticateToken, (req, res) => {
...
// 删除健康记录
router.delete('/records/:date', authenticateToken, (req, res) => {
    const { date } = req.params;
    ...
    db.run(
        ...
        function(err) {
            ...
            res.json({ message: 'Health record deleted successfully', affected: this.changes });
        }
    );
});
