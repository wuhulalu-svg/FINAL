...
// ========== 发送注册验证码 ==========
router.post('/send-register-code', (req, res) => {
  ...
  db.get('SELECT id FROM users WHERE email = ?', [email], async (err, row) => {
    ...
    if (row) return res.status(400).json({ error: '该邮箱已注册' });
...
router.get('/me', authenticateToken, (req, res) => {
  db.get(
    'SELECT id, name, email, age, gender, height, weight, role FROM users WHERE id = ?',
    [req.user.id],
    (err, user) => {
      if (err) {
        console.error('数据库错误:', err);
