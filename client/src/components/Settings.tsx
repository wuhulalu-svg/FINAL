...
import { User, HealthGoal, HealthRecord } from '../App';
...
export function Settings({ user, healthGoals, onAddGoal, onUpdateGoal, onDeleteGoal }: SettingsProps) {
  ...
  const handleAddGoal = () => {
    ...
    const metric = GOAL_METRICS.find(m => m.key === newGoal.metric);
    
    const goal = {
      metric: newGoal.metric,
      metricLabel: metric?.label || newGoal.metric,
  ...
  const activeGoals = healthGoals.filter(g => {
    const today = new Date().toISOString().split('T')[0];
    return today >= g.startDate && today <= g.endDate;
  });

  const completedGoals = healthGoals.filter(g => {
    const today = new Date().toISOString().split('T')[0];
    return today > g.endDate;
  });

  return (
    <div className="max-w-5xl mx-auto">
      <header className="mb-8">
        <h1 className="text-gray-800 dark:text-white text-2xl font-bold mb-2">{text.settingsAndGoals}</h1>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-gray-800 dark:text-white font-semibold">{text.profileInformation}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">{text.name}</p>
            ...
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">{text.age}</p>
              <p className="text-gray-800 dark:text-white">{user?.age} {text.years}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">{text.gender}</p>
              <p className="text-gray-800 dark:text-white capitalize">{user?.gender}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">{text.height}</p>
              <p className="text-gray-800 dark:text-white">{user?.height} {text.cm}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">{text.weight}</p>
              <p className="text-gray-800 dark:text-white">{user?.weight} {text.kg}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-gray-800 dark:text-white font-semibold">{text.language}</h2>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-gray-800 dark:text-white font-semibold">{text.healthGoals}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
          ...
          {activeGoals.length > 0 ? (
            <div className="space-y-3 mb-6">
              ...
              {activeGoals.map(goal => {
                ...
                const daysElapsed = Math.floor(
                  (new Date(today).getTime() - new Date(goal.startDate).getTime()) / (1000 * 60 * 60 * 24)
                );
                ...
                return (
                  <div key={goal.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h4 className="text-gray-800 dark:text-white font-medium">{goal.metricLabel}</h4>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Target: {goal.targetValue}</p>
                      </div>
                      <button
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                      ...
                      {goal.missedDays.length > 0 && (
                        <span className="px-2 py-0.5 bg-yellow-100 dark:bg-yellow-900/50 text-yellow-700 dark:text-yellow-400 rounded">
                          {goal.missedDays.length} missed day{goal.missedDays.length > 1 ? 's' : ''}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              ...
              <p>No active goals. Click "Add Goal" to create one!</p>
            </div>
          )}
                ...
                {completedGoals.map(goal => (
                  <div key={goal.id} className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-3 opacity-60">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="text-gray-700 dark:text-gray-400 text-sm">{goal.metricLabel}</h4>
                        <p className="text-xs text-gray-500 dark:text-gray-500">Target: {goal.targetValue}</p>
                      </div>
                      <span className="text-xs text-gray-500 dark:text-gray-500">
                        Ended {new Date(goal.endDate).toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' })}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
          <h2 className="text-gray-800 dark:text-white font-semibold mb-4">{text.notificationPreferences}</h2>
          <div className="space-y-3">
            <label className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
              <span className="text-gray-700 dark:text-gray-300">{text.goalMissedReminders}</span>
              <input type="checkbox" defaultChecked className="w-5 h-5 text-indigo-600 rounded" />
            </label>
            <label className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
              <span className="text-gray-700 dark:text-gray-300">{text.healthAnomalyAlerts}</span>
              <input type="checkbox" defaultChecked className="w-5 h-5 text-indigo-600 rounded" />
            </label>
            <label className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
              <span className="text-gray-700 dark:text-gray-300">{text.dailyHealthSummary}</span>
